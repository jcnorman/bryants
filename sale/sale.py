...
class Sale(Workflow, ModelSQL, ModelView):
    ...
    work_date = fields.Date('Work Date',
        ...
        #     },
        depends=['state']),
    work_by = fields.Many2One('company.employee', 'Work by', depends = ['company']),
    ...
    internal_comment = fields.Text('Office Comment')
    ...
    delivery_date = fields.Date('Delivery Date',
        states={
            'readonly': ~Eval('state').in_(['draft', 'quotation', 'confirmed', 'processing']),
            'required': ~Eval('state').in_(['draft', 'quotation', 'cancel', 'confirmed']),
            },
        depends=['state'])
#    quotation = fields.Many2One('quotation.master', 'Quotation')
...
#            'required': Eval('type') == 'line',
    ...
    quote_amount = fields.Numeric('Quote Amount', digits = (16,2),
                                  readonly=True)
    amount = fields.Numeric('Amount',
            digits=(16, Eval('_parent_sale', {}).get('currency_digits', 2)),
            states={
                'invisible': ~Eval('type').in_(['line', 'subtotal']),
#                'readonly': Eval('type') != 'line',
                },
            depends=['type'])
    description = fields.Text('Description', size=None, required=True)
    note = fields.Text('Note')
    internal_note = fields.Text('Internal Comment')
